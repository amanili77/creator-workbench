...
async function saveSettings() {
    ...
    try { const result=await api("/api/settings","PUT",payload); if(!result.ok) throw new Error(result.error||"保存失败"); status.textContent="保存成功，正在刷新界面"; loadedSettings=result.config; setTimeout(()=>window.location.reload(),500); }
